...
def extract_text():
    ...
    user_name = request.form[
        "user_name"
    ]  # O nome do usuário será enviado diretamente pelo front-end

    file_extension = file.filename.split(".")[-1].lower()

    print(
        f"Recebendo arquivo: {file.filename}, extensão: {file_extension}, usuário: {user_name}"
    )
    ...
    def preprocess_image(image):
        # Reduz o tamanho da imagem (ajuste o valor de scale_factor conforme necessário)
    ...
    if file_extension in ["jpg", "jpeg", "png"]:
        ...
        for image in images:
            ...
            text += (
                pt.image_to_string(processed_img, lang="por", config="--psm 6") + " "
            )
...
def extract_course_name(text):
    ...
    if match:
        ...
        course_name = re.sub(
            r" -.*| promovido.*| patrocínio.*| apoio.*| realizado.*| com.*",
            "",
            course_name,
        )
        return course_name
...
def extract_course_name_from_quotes(text):
    ...
    for match in matches:
        ...
        if (
            len(match_clean.split()) > 1
        ):  # Certifica que o nome do curso tem mais de uma palavra
            return match_clean
...
def extract_duration_or_calculate(text):
    ...
    date_range_match = re.search(
        r"(\d{2}[/-]\d{2}[/-]\d{4})\s*[aà]\s*(\d{2}[/-]\d{2}[/-]\d{4})", text
    )
    if date_range_match:
        ...
        try:
            ...
            hours = (
                days_difference * 8
            )  # Ajuste o valor 8 para a carga horária padrão desejada
    ...
    try:
        certificados = list(
            db.certificados.find(
                {"funcionario_id": ObjectId(funcionario_id)}, {"_id": 0}
            )
        )  # Filtra por funcionario_id
        if certificados:
            return jsonify(certificados), 200
        return (
            jsonify({"error": "Nenhum certificado encontrado para este funcionário."}),
            404,
        )
    except Exception as e:
    ...
    try:
        ...
        if funcionario:
            if check_password_hash(funcionario["senha"], str(senha)):
                ...
                return (
                    jsonify(
                        {"message": "Login bem-sucedido!", "funcionario": funcionario}
                    ),
                    200,
                )
            else:
...
def create_funcionario():
    ...
    if not photo:
        return (
            jsonify(
                {
                    "error": "Formato de imagem inválido. Apenas PNG, JPG e JPEG são permitidos."
                }
            ),
            400,
        )
    ...
    novo_funcionario = {
        "nome": data.get("nome"),
        "usuario": data.get("usuario"),
        "photo": photo,  # Salva o caminho da foto
...
def get_photo(usuario):
    ...
    funcionario = db.funcionarios.find_one(
        {"usuario": usuario}, {"_id": 0}
    )  # Exclui o campo _id na resposta
        ...
        if "photo" in funcionario and funcionario["photo"]:
            ...
            try:
                ...
                if photo_bytes:
                    ...
                    image = image.convert(
                        "RGB"
                    )  # Converte para RGB caso a imagem seja PNG ou outro formato
                    output_io = io.BytesIO()
                    image.save(
                        output_io, format="JPEG", quality=85
                    )  # Ajuste a qualidade da compressão conforme necessário
                    output_io.seek(0)

                    # Retornar a imagem compactada como binário
                    return Response(
                        output_io.read(), mimetype="image/jpeg"
                    )  # Envia a imagem compactada diretamente como binário
                else:
...
def get_funcionario(usuario):
    # Buscar o funcionário pelo nome de usuário
    funcionario = db.funcionarios.find_one(
        {"usuario": usuario}, {"_id": 0, "photo": 0}
    )  # Exclui o campo _id e 'photo' na resposta
...
def update_funcionario(usuario):
    ...
    updated_data = {
        ...
        "tipo_funcionario": data.get(
            "tipo_funcionario", funcionario["tipo_funcionario"]
        ),
        "cargo": {
            "nome_cargo": data.get("cargo", {}).get(
                "nome_cargo", funcionario["cargo"].get("nome_cargo")
            ),
            "salario": data.get("cargo", {}).get(
                "salario", funcionario["cargo"].get("salario")
            ),
        },
    }
    ...
    if "photo" in request.files:
        ...
        if isinstance(
            photo, FileStorage
        ):  # Certificando-se de que o 'photo' é um arquivo válido
            try:
